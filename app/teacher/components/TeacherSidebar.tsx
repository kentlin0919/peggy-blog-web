  ...
  const menuItems = [
    {
      ...
      items: [
        ...
        { name: '預約管理', href: '/teacher/bookings', icon: 'calendar_month', badge: '3' },
        { name: '課程方案', href: '/teacher/courses', icon: 'school' },
        { name: '作品集管理', href: '/teacher/portfolio', icon: 'photo_library' },
            ...
            <nav className="flex flex-col gap-1.5 overflow-y-auto">
              {menuItems.map((category, idx) => (
                <div key={idx} className={idx > 0 ? "mt-6" : ""}>
                    <p className="px-3 text-xs font-semibold text-text-sub uppercase tracking-wider mb-2">{category.category}</p>
