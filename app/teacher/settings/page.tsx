...
export default function TeacherSettingsPage() {
  ...
  // Notification State
  const [notifNewBooking, setNotifNewBooking] = useState({ email: true, app: true, line: true });
  const [notifReminder, setNotifReminder] = useState({ email: false, app: true, line: true });
  const [notifCancel, setNotifCancel] = useState({ email: true, app: true, line: true });
  const [notifSystem, setNotifSystem] = useState({ email: true, app: true, line: false });
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (userData) {
        ...
        setAvatarUrl(userData.avatar_url || 'https://lh3.googleusercontent.com/aida-public/AB6AXuDO6reZcrIx4TpNaoa2cHckBMT5jGOVcSOgCiWFeFHvwR5BhOlm6EzZoO1nDA5jhhdVnLiS3xPcbfPeCVuaPW7x9yyQ1OpilXHhQqZf7s1ilC_fOFoonIf98HRVehAYuVriM8l3I0MrYHIn39RVWEj_4jU-wlh_BemOK4VeRUNedhA-sln2p5816fNCRlBCziM3mk1IHmY1EIx1yw45MJkIcGFi9fz7JcPrVe1C0mU8MYnl7CYlnU1BMQEyHUmuypSHuwydpWLgPOU');
      }
      ...
      if (teacherData) {
        ...
        if (notifSettings.new_booking) setNotifNewBooking(notifSettings.new_booking);
  ...
  const handlePauseService = async () => {
    ...
    if (!confirm(`確定要${action}服務嗎？`)) return;
    ...
    <>
      <header className="w-full bg-surface-light/80 dark:bg-surface-dark/80 backdrop-blur-md border-b border-border-light dark:border-border-dark px-8 py-4 flex justify-between items-center sticky top-0 z-10 transition-all">
        <div className="flex flex-col">
          ...
          <p className="text-text-sub dark:text-gray-400 text-sm mt-0.5">管理您的個人資料、預約規則與通知偏好</p>
        </div>
        ...
        <div className="max-w-[1000px] mx-auto flex flex-col gap-8 pb-10">
          ...
          <section className="flex flex-col gap-6" id="personal">
            <div className="flex items-center gap-3 pb-2 border-b border-border-light dark:border-border-dark">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">個人設定</h3>
            </div>
              ...
              <div className="md:col-span-1">
                <div className="flex flex-col items-center gap-4 p-6 bg-surface-light dark:bg-surface-dark rounded-2xl border border-border-light dark:border-border-dark shadow-sm">
                  <div className="relative group">
                    ...
                    ></div>
                    <button className="absolute bottom-0 right-0 p-2 bg-primary text-white rounded-full shadow-lg hover:bg-primary-dark transition-colors" title="更換頭像">
                      <span className="material-symbols-outlined text-sm">edit</span>
                    </button>
                  </div>
                  <div className="text-center">
                    <h4 className="font-bold text-slate-800 dark:text-white">{name}</h4>
                    <p className="text-xs text-text-sub dark:text-gray-400 mt-1">ID: {teacherId}</p>
                  </div>
                </div>
              </div>
              <div className="md:col-span-2 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700 dark:text-gray-300">顯示名稱</label>
                    <input 
                      value={name}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-sm font-medium text-slate-700 dark:text-gray-300">專業頭銜</label>
                    <input 
                      value={title}
          ...
          <section className="flex flex-col gap-6" id="booking">
            <div className="flex items-center gap-3 pb-2 border-b border-border-light dark:border-border-dark">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">預約與時間設定</h3>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div className="flex items-center gap-2 mb-2 pb-2 border-b border-border-light dark:border-border-dark md:border-b-0">
                    ...
                    <h4 className="text-sm font-bold text-slate-800 dark:text-white">預約規則設定</h4>
                  </div>
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-700 dark:text-gray-300">開放預約範圍</label>
                      <p className="text-xs text-text-sub mb-1">學生可預約多久以後的課程</p>
                      <select 
                        value={bookingWindow}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-700 dark:text-gray-300">預約間隔緩衝</label>
                      <p className="text-xs text-text-sub mb-1">每堂課之間的休息時間</p>
                      <select 
                        value={bookingBuffer}
                ...
                <div className="space-y-6 md:pl-8 md:border-l md:border-border-light md:dark:border-border-dark">
                  <div className="flex items-center gap-2 mb-2 pb-2 border-b border-border-light dark:border-border-dark md:border-b-0">
                    <span className="material-symbols-outlined text-text-sub">edit_calendar</span>
                    <h4 className="text-sm font-bold text-slate-800 dark:text-white">取消與改期政策</h4>
                  </div>
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-slate-700 dark:text-gray-300">最晚取消時間</label>
                      <select 
                        value={cancelPolicy}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <label className="text-sm text-slate-600 dark:text-gray-300" htmlFor="penalty">
                        啟用頻繁取消限制
                        <span className="block text-xs text-text-sub mt-0.5">一週內取消超過 3 次將暫停預約權限</span>
                      </label>
                    </div>
                    <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-900/30 rounded-xl p-4">
                      <div className="flex gap-3">
                        <span className="material-symbols-outlined text-yellow-600 dark:text-yellow-500 text-xl shrink-0">lightbulb</span>
                        <div className="text-sm">
                          <h5 className="font-bold text-yellow-800 dark:text-yellow-400 mb-1">小提示</h5>
                          <p className="text-yellow-700 dark:text-yellow-300/80 leading-relaxed text-xs">設定合理的取消政策有助於保障您的教學權益。建議在課程說明中也同步更新這些規則。</p>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <section className="flex flex-col gap-6" id="notification">
            <div className="flex items-center gap-3 pb-2 border-b border-border-light dark:border-border-dark">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">通知規則</h3>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-50 dark:bg-slate-800 border-b border-border-light dark:border-border-dark">
                      <th className="px-6 py-4 text-xs font-semibold text-text-sub uppercase tracking-wider">觸發事件</th>
                      <th className="px-6 py-4 text-xs font-semibold text-text-sub uppercase tracking-wider text-center">電子郵件</th>
                      <th className="px-6 py-4 text-xs font-semibold text-text-sub uppercase tracking-wider text-center">站內信通知</th>
                      <th className="px-6 py-4 text-xs font-semibold text-text-sub uppercase tracking-wider text-center">LINE 通知</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border-light dark:divide-border-dark">
                    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-slate-800 dark:text-white">新預約成立</span>
                          <span className="text-xs text-text-sub">當學生提交新的課程預約申請時</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifNewBooking.email} onChange={e => setNotifNewBooking({...notifNewBooking, email: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifNewBooking.app} onChange={e => setNotifNewBooking({...notifNewBooking, app: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifNewBooking.line} onChange={e => setNotifNewBooking({...notifNewBooking, line: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-slate-800 dark:text-white">課程前提醒</span>
                          <span className="text-xs text-text-sub">課程開始前 2 小時發送提醒</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                         <input checked={notifReminder.email} onChange={e => setNotifReminder({...notifReminder, email: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifReminder.app} onChange={e => setNotifReminder({...notifReminder, app: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifReminder.line} onChange={e => setNotifReminder({...notifReminder, line: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-slate-800 dark:text-white">預約取消/變更</span>
                          <span className="text-xs text-text-sub">當學生取消或修改預約時間時</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifCancel.email} onChange={e => setNotifCancel({...notifCancel, email: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifCancel.app} onChange={e => setNotifCancel({...notifCancel, app: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifCancel.line} onChange={e => setNotifCancel({...notifCancel, line: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-slate-800 dark:text-white">系統公告</span>
                          <span className="text-xs text-text-sub">平台發布的重要維護或功能更新</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifSystem.email} onChange={e => setNotifSystem({...notifSystem, email: e.target.checked})} disabled className="rounded text-primary focus:ring-primary border-gray-300 size-4 opacity-50 cursor-not-allowed" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifSystem.app} onChange={e => setNotifSystem({...notifSystem, app: e.target.checked})} disabled className="rounded text-primary focus:ring-primary border-gray-300 size-4 opacity-50 cursor-not-allowed" type="checkbox" />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <input checked={notifSystem.line} onChange={e => setNotifSystem({...notifSystem, line: e.target.checked})} className="rounded text-primary focus:ring-primary border-gray-300 size-4" type="checkbox" />
                      </td>
                    </tr>
                  </tbody>
                </table>
          ...
          <section className="flex flex-col gap-6" id="integrations">
            <div className="flex items-center gap-3 pb-2 border-b border-border-light dark:border-border-dark">
              ...
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">其他平台整合</h3>
            </div>
                ...
                <div className="flex items-start justify-between z-10">
                  <div className="flex items-center gap-3">
                    <div className="size-10 rounded-full bg-white dark:bg-slate-800 border border-border-light dark:border-border-dark flex items-center justify-center p-2">
                      ...
                      <img alt="Google Calendar" className="w-full h-full object-contain" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDaa9q-lq_AGaRtkDR3GfsitRjPx-qG6UggEorRIHpejSfmyipRb7OIECGo9KkXHTHSxpX3yyeYU_Gs4Zj0B8HfXbmkxl6OkSeZsOcxfxFIR5e4V902heo0lahNSHn2DsKOLSv2DNtkyW9uSWChBVBSQx-8DukdjmHaBazd9QzUWz8yJ39tAfXcXs2WKyxnaOLXJhMHT2_y6-ncP8y1bucgP7DzMXATtIxHW4ddEueOz9KuBjlgDkY6Nf9T_o5JkvoXq6MJzctkdGQ" />
                    </div>
                    <div>
                      <h4 className="font-bold text-slate-800 dark:text-white">Google 日曆</h4>
                      <p className="text-xs text-text-sub">同步課程時間到您的個人日曆</p>
                    </div>
                  </div>
              ...
              <div className="bg-surface-light dark:bg-surface-dark rounded-2xl border border-border-light dark:border-border-dark p-6 flex flex-col gap-4 shadow-sm relative overflow-hidden">
                ...
                <div className="flex items-start justify-between z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-800 dark:text-white">LINE 通知</h4>
                      <p className="text-xs text-text-sub">接收即時預約與提醒通知</p>
                    </div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input 
