        ...
        <div className="flex flex-col h-full p-6">
          ...
          <nav className="flex flex-col gap-2 flex-1 overflow-y-auto">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 px-4">
              主選單 ({userRole === 'student' ? '學生' : userRole === 'teacher' ? '教師' : '訪客'})
            </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`material-symbols-outlined text-[22px] ${!isActive && 'group-hover:scale-110 transition-transform'}`}>
                    {item.icon}
                  </span>
          ...
          <div className="mt-auto pt-6 border-t border-slate-100 dark:border-slate-800">
            {userRole ? (
               ...
               >
                ...
                <Link href="/auth/login" className="flex items-center gap-3 p-2 rounded-xl hover:bg-teal-50 hover:text-teal-600 transition-all text-slate-600">
