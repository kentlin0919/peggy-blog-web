  ...
  const fetchProfile = async () => {
    try {
      ...
      if (userInfo) {
        ...
        // Bio and School are not in DB, leaving blank or defaults
      }
    } catch (error) {
        ...
        <div className="lg:col-span-1 flex flex-col gap-6 sticky top-10">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/10 rounded-2xl p-5 border border-blue-100 dark:border-blue-900/20">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-bold text-blue-900 dark:text-blue-100 text-sm mb-1">完成個人資料</h4>
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <div className="bg-surface-light dark:bg-surface-dark bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-soft p-6 md:p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">基本資料</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">更新您的姓名與聯絡方式</p>
              </div>
              <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">真實姓名</label>
                <input 
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">手機號碼</label>
                <input 
                    className="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-primary focus:ring-primary trans-all font-medium" 
                    type="tel" 
                    value={phone}
              ...
              {/* Note: 'School/Grade' and 'Bio' fields are removed/hidden as they don't map to DB columns currently */}
            </div>
          </div>
          ...
          <div className="bg-surface-light dark:bg-surface-dark bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-soft p-6 md:p-8">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-bold text-slate-800 dark:text-white">帳戶安全</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400">定期更改密碼以保護您的帳戶</p>
              </div>
            </div>
            <div className="space-y-6 max-w-lg">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">新密碼</label>
                  <input 
                    className="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-primary focus:ring-primary trans-all font-medium" 
                    placeholder="請輸入新密碼" 
                    type="password"
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">確認新密碼</label>
                  <input 
                    className="w-full rounded-xl border-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-white focus:border-primary focus:ring-primary trans-all font-medium" 
                    placeholder="再次輸入新密碼" 
                    type="password"
          ...
          <div className="bg-surface-light dark:bg-surface-dark bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-soft p-6 md:p-8">
            <div className="mb-6">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">通知偏好</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">自定義您希望接收的訊息類型</p>
            </div>
            <div className="space-y-6 divide-y divide-slate-100 dark:divide-slate-800">
              <div className="flex items-center justify-between pt-2">
                <div>
                  <h4 className="font-bold text-slate-700 dark:text-slate-200">課程提醒</h4>
                  <p className="text-xs text-slate-500 mt-1">在上課前 24 小時發送 Email 與簡訊提醒</p>
                </div>
              ...
              <div className="flex items-center justify-between pt-6">
                <div>
                  <h4 className="font-bold text-slate-700 dark:text-slate-200">學習進度報告</h4>
                  <p className="text-xs text-slate-500 mt-1">每週寄送一次您的練習時數與課程進度總結</p>
                </div>
                <label className="relative inline-flex items-center cursor-pointer">
                  <input defaultChecked className="sr-only peer" type="checkbox" />
                  <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/30 dark:peer-focus:ring-primary/20 rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-primary"></div>
          ...
          <div className="mt-4 pt-8 border-t border-slate-200 dark:border-slate-800">
            ...
            <div className="flex items-center justify-between bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 p-4 rounded-xl">
              <div>
                <h4 className="font-bold text-red-600 dark:text-red-400 text-sm">刪除帳戶</h4>
                <p className="text-xs text-red-400/80 mt-0.5">此動作無法復原，您的所有課程進度將被永久刪除。</p>
              </div>
              <button 
